...
> 🆕 **本地会话自动捕获** —— 在终端里直起的 `claude` / `codex` 会话现在会自动出现在 AgentQuad 看板上，并按 config 默认通知路由推送到飞书 / Telegram。如果 `~/.claude` 或 `~/.codex` 已存在，启动 server 时会自动装/升级所需的 hook。不需要的话在 `~/.agentquad/config.json` 把 `localSessions.autoInstallHooks: false` 关掉即可。若自动安装失败（如权限问题），web 端会弹一条可关闭的 banner 提醒你手动跑 `agentquad install claude`。详见 [`docs/LOCAL-SESSIONS.md`](./docs/LOCAL-SESSIONS.md)。
