...
> ⚠️ **本地会话自动捕获（新）** —— 如果你在旧版本跑过 `agentquad install claude`，升级后**需要重新跑一次**。新版本 hook 事件集多了 `SessionStart`，并升级了 hook 脚本版本标记。不重装的话，本地终端直起的 `claude` 会话不会在 web 端自动建卡。如果 web 端检测到 hook 版本过旧，会在顶部显示一条可关闭的警告 banner。详见 [`docs/LOCAL-SESSIONS.md`](./docs/LOCAL-SESSIONS.md)。
